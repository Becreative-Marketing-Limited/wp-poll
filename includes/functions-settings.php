...
if ( ! function_exists( 'wpp_register_plugin_settings' ) ) {
	function wpp_register_plugin_settings() {
		$pages = array();
		...
		wpp()->PB_Settings( array(
			...
			'pages'           => apply_filters( 'wpp_filters_settings_pages', $pages ),
		) );
	}
}
add_action( 'init', 'wpp_register_plugin_settings' );