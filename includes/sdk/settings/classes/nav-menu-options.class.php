 ...
 *
 */
if ( ! class_exists( 'PBSettings_Nav_Menu_Options' ) ) {
  class PBSettings_Nav_Menu_Options extends PBSettings_Abstract{
    ...
    public function wp_edit_nav_menu_walker( $class, $menu_id ) {
      ...
      if( version_compare( $wp_version, '5.4.0', '<' ) ) {
        ...
        if ( ! class_exists( 'PBSettings_Walker_Nav_Menu_Edit' ) ) {
          PBSettings::include_plugin_file( 'functions/walker.php' );
        }

        return 'PBSettings_Walker_Nav_Menu_Edit';
    ...
    public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
      ...
      echo '<div class="pb_settings pbsettings-nav-menu-options'. esc_attr( $class ) .'">';
        ...
        foreach ( $this->sections as $section ) {
          ...
          echo ( $section_title || $section_icon ) ? '<div class="pbsettings-nav-menu-title"><h4>'. $section_icon . $section_title .'</h4></div>' : '';
          ...
          if ( ! empty( $section['fields'] ) ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              PBSettings::field( $field, $this->get_meta_value( $menu_item_id, $field ), $this->unique .'['. $menu_item_id .']', 'menu' );
    ...
    public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $menu_item_db_id, '_pb_settings_errors_'. $this->unique, $errors );
        }
