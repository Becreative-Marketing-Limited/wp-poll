 ...
 *
 */
if ( ! class_exists( 'PBSettings_Taxonomy_Options' ) ) {
	class PBSettings_Taxonomy_Options extends PBSettings_Abstract {
				...
				if ( $taxonomy === $this->taxonomy ) {
					...
					if ( ! empty( $section['fields'] ) ) {
						foreach ( $section['fields'] as $field ) {
							...
							PBSettings::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
		...
		public function save_taxonomy( $term_id ) {
			...
			if ( empty( $data ) ) {
				...
				if ( ! empty( $errors ) ) {
					update_term_meta( $term_id, '_pb_settings_errors_' . $this->unique, $errors );
				}
