 ...
 *
 */
if ( ! class_exists( 'PBSettings_Shortcoder' ) ) {
  class PBSettings_Shortcoder extends PBSettings_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'PBSettings Shortcodes',
        'description'    => 'PBSettings Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        PBSettings::$shortcode_instances[] = wp_parse_args( array( 'name' => 'pb_settings/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( PBSettings::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'PBSettings', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'PBSettings_Field_icon', 'add_footer_modal_icon' ) );
        ...
        <div class="pbsettings-modal-table">
          <div class="pbsettings-modal-table-cell">
            ...
            <div class="pbsettings-modal-inner">
              ...
              <div class="pbsettings-modal-insert-wrapper hidden"><a href="#" class="button button-primary pbsettings-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              PBSettings::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            echo '<div class="pbsettings--repeatable">';
              ...
              echo '<div class="pbsettings--repeat-shortcode">';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  PBSettings::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="pbsettings--repeat-button-block"><a class="button pbsettings--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. $button_title .'</a></div>';
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'PBSettings_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( pb_settings_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'PBSettings_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'pbsettings-gutenberg-block', PBSettings::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'pbsettings-gutenberg-block', 'pb_settings_gutenberg_blocks', PBSettings::$shortcode_instances );

      foreach ( PBSettings::$shortcode_instances as $block ) {
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( PBSettings::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary pbsettings-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. $value['button_title'] .'</a>';
      }
