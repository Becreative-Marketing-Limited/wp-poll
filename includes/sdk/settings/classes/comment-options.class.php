 ...
 *
 */
if ( ! class_exists( 'PBSettings_Comment_Metabox' ) ) {
	class PBSettings_Comment_Metabox extends PBSettings_Abstract {
			...
			foreach ( $this->sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						PBSettings::field( $field, $this->get_meta_value( $comment->comment_ID, $field ), $this->unique, 'comment_metabox' );
		...
		public function save_comment_meta_box( $comment_id ) {
			...
			if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
				...
				if ( ! empty( $errors ) ) {
					update_comment_meta( $comment_id, '_pb_settings_errors_' . $this->unique, $errors );
				}
